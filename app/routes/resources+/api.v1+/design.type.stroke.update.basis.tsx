...
import {
	...
	validateDesignTypeUpdateStrokeBasisSubmission,
...
export async function action({ request }: ActionFunctionArgs) {
	...
	const { status, submission } =
		await validateDesignTypeUpdateStrokeBasisSubmission({
			userId,
			formData,
		})

	if (status === 'success') {
		const { success } = await updateDesignTypeStrokeBasis({
			userId,
			...submission.value,
		})
...
export const DesignTypeStrokeBasis = ({
	design,
	formLocation = '',
}: {
	...
	const options = schemaEnumToSelectOptions(StrokeBasisTypeEnum)
