...
const validateQueryWhereArgsPresent = (where: queryWhereArgsType) => {
	...
	if (Object.keys(missingValues).length > 0) {
		...
		throw new Error(
			'Null or undefined values are not allowed in query parameters for design size.',
		)
	}
}

export const getDesignSize = async ({
	where,
}: {
	where: queryWhereArgsType
}): Promise<IDesignSize | null> => {
	...
	invariant(design, 'Design Size Not found')
	return deserializeDesign({ design }) as IDesignSize
}
